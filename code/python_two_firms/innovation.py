...
class Aiyagari:
    ...
    def find_equilibrium(self, agrid, zgrid, R, w, p):
        ...
        # Find policies
        gc, ga, π, v = find_policies(agrid, zgrid, R, w, p)
        ...
        return gc, ga, v, π, T, λ, λ_a

    def demand(self, agrid, zgrid, R, w, p):
        find_policies = self.find_policies
        find_transition_matrix = self.find_transition_matrix
        find_stationary_distribution = self.find_stationary_distribution
        ...
        π_p = π.reshape([agrid.size*zgrid.size, self.N], order='F').T @ λ
            
        return π_p

    def elasticity(self, agrid, zgrid, R, w, p):
        demand = self.demand
        N = self.N

        d = lambda p0: demand(agrid, zgrid, R, w, p0)

        π = d(p)
        J = jacobian(d, 1e-10, p)
    ...
    def marginal_cost(self, w, z):
        return w/z

    def prices_fp(self, agrid, zgrid, R, w, z_firm, p):
        markup = self.markup
        ...
        # Marginal cost 
        mc = marginal_cost(w, z_firm)

        # Elasticity
        ε = elasticity(agrid, zgrid, R, w, p)

        # Residual
        return markup(ε) * mc - p
