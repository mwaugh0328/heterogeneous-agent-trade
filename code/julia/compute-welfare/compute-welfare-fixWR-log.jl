...
Δ_p = make_p(Δ_Wsol[1:end], TFP, d_prime[home_country, :], cntry_prm.tariff[home_country, :] )
...
Δ_hh = solve_household_problem(Δ_Rsol[19], Δ_Wsol[19], p , 0.0, foo_hh_prm, tol = 1e-10)
...
λτeqv =  eq_variation_porportional(R, W, p, Δ_hh, dist[home_country].state_index, foo_hh_prm)[1]

writedlm("./output/welfare-fixWR-log-flip.txt", λτeqv)
...
root = rootfile*"us-cross-section-fixWR-log-flip.csv"
