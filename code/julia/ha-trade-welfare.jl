function eq_variation_porportional(R, w, p, Δ_hh, state_index, model_params)
...
function eq_variation_porportional(x, astate, shockstate, R, w, p, Δ_v, model_params)
    ...
    foo_hh_params = household_params(model_params, λτ = x[1])

    τ = 0.0

    eqv_hh = solve_household_problem(R, w, p, τ, foo_hh_params, tol = 10^-12)
...
function eq_variation_lumpsum(R, w, p, Δ_hh, state_index, model_params)
    # these should be the old prices
    # Δ_hh is the value function at the new prices
    # this works through everything state by state
    ...
    for (foo, xxx) in enumerate(state_index)
        ...
        f(x) = eq_variation_lumpsum(x, xxx[1], xxx[2], R, w, p, Δ_v[xxx[1], xxx[2]], model_params)
...
function eq_variation_lumpsum(xxx, astate, shockstate, R, w, p, Δ_v, model_params)
    ...
    eqv_hh = solve_household_problem(R, w, p, x[1], model_params)
