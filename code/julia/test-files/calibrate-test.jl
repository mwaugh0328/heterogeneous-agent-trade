...
moments = diag(tradesharedata)

f(x) = calibrate_world_equillibrium(x, moments, mdl_prm);
...
initial_x = [ones(Ncntry-1); R; log.(TFP[1:end])]
...
Rsol = sol.x[Ncntry:2*Ncntry-1]
TFPsol = sol.x[2*Ncntry:end]

mdl_prm = world_model_params(mdl_prm, TFP = TFPsol)
