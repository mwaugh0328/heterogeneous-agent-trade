...
Again, I' the trade elasticity defined as
\begin{align}
\frac{\partial ( M_{ij} / M_{ii} )}{\partial d_{ij}} \times \frac{d_{ij}}{( M_{ij} / M_{ii} )} =& \left(\frac{M_{ij}'M_{ii} - M_{ij}M_{ii}'}{M_{ii}^2}\right)\left(\frac{M_{ii}}{M_{ij}}\right)d_{ij} \\
\nonumber \\
=& \left(\frac{M_{ij}'}{M_{ij}} - \frac{M_{ii}'}{M_{ii}}\right)d_{ij}
\end{align}
which is the difference in elasticities between how trade between $i$ and $j$ change relative to home trade. So let's compute how trade changes
\begin{align}
...
\end{align}
and then from here divide the inside of the brackets by $p_{ij}c_{ij}(a,z)\pi_{ij}(a,z) \lambda(a,z)$ and multiply the outside which gives
\begin{align}
...
\end{align}
then define the following ``weight'' which is the share of goods that those with states $a,z$ account for in total expenditures from $j$ giving
\begin{align}
\omega_{ij}(a,z) = \frac{p_{ij}c_{ij}(a,z)\pi_{ij}(a,z) \lambda(a,z)}{M_{ij}}
\end{align}
then from this definition we have
\begin{align}
\frac{\partial  M_{ij} / M_{ij}}{\partial d_{ij} / d_{ij}} = \sum_{a,z} \bigg \{ \frac{\partial p_{ij}c_{ij}(a,z)/ p_{ij}c_{ij}(a,z)}{\partial d_{ij} / d_{ij}}+
\frac{\partial \pi_{ij}(a,z) / \pi_{ij}(a,z)}{\partial d_{ij} / d_{ij}}  +
\frac{\partial \lambda(a,z) / \lambda(a,z)}{\partial d_{ij} / d_{ij}} \bigg \} \omega_{ij}(a,z)
\end{align}
which I'll rearrange and label the following way
\begin{align}
\frac{\partial  M_{ij} / M_{ij}}{\partial d_{ij} / d_{ij}} = & \sum_{a,z} \underbrace{\bigg \{ \frac{\partial p_{ij}c_{ij}(a,z)/ p_{ij}c_{ij}(a,z)}{\partial d_{ij} / d_{ij}} \bigg \}}_{\theta_{ij}(a,z)^{I}}\omega_{ij}(a,z)
+ \sum_{a,z} \underbrace{\bigg \{ \frac{\partial \pi_{ij}(a,z) / \pi_{ij}(a,z)}{\partial d_{ij} / d_{ij}} \bigg \}}_{\theta_{ij}(a,z)^{E}} \omega_{ij}(a,z) \\
\nonumber \\
& \ \ \ + \ \underbrace{ \bigg \{\sum_{a,z} \frac{\partial \lambda(a,z) / \lambda(a,z)}{\partial d_{ij} / d_{ij}} \bigg \}}_{\theta_{ij}(a,z)^{D}}  \omega_{ij}(a,z)
\end{align}
Or in other words \textbf{IED}
\begin{align}
...
\end{align}
where the aggregate elasticity of imports from $j$ is a weighted average of elasticities at the household level. And household level elasticities are decomposed into the following components
\begin{itemize}
  \item \textbf{I:} The household level intensive margin trade elasticity $\theta(a,z)_{ij}^{I}$
  \item \textbf{E:} The household level extensive margin trade elasticity $\theta(a,z)_{ij}^{E}$
  \item \textbf{D:} The distribution elasticity $\theta(a,z)_{ij}^{D}$, i.e. how the mass of agents changes across household states $a,z$.
\end{itemize}
...
\textbf{Intensive Margin Trade Elasticity} This can be determined by just working off the budget constraint so
\begin{align}
\underbrace{\frac{\partial p_{ij}c_{ij}(a,z)/ p_{ij}c_{ij}(a,z)}{\partial d_{ij} / d_{ij}}}_{\theta_{ij}(a,z)^{I}} &= -\frac{\partial a(a,z)'}{\partial d_{ij}/ d_{ij}} \times \frac{1}{p_{ij}c_{ij}(a,z)}
\end{align}
where the key issue determining the intensive margin elasticity is how the household trades off assets for a change in trade costs. Just to be clear how this works: a reduction in trade costs lowers prices and relaxes the budget constraint, so how the incremental amount of resources available to spend is dived between assets and consumption determines the intensive margin trade elasticity. Here are two special cases:
\begin{itemize}
...
\textbf{Extensive Margin Trade Elasticity} This where the Type 1 extreme value functional form matters. First, I'm going to define the denominator term of the choice probability as the following:
\begin{align}
\Phi_{i}(a,z) = \sum_{j'} \exp \left( \frac{ v_{ij'}(a, z) }{\sigma_{\epsilon}} \right)
\end{align}
Then the elasticity of the choice probability with respect to a change in trade costs is 
\begin{align}
\underbrace{\bigg \{ \frac{\partial \pi_{ij}(a,z) / \pi_{ij}(a,z)}{\partial d_{ij} / d_{ij}} \bigg \}}_{\theta_{ij}(a,z)^{E}} &= \frac{1}{\sigma}\frac{\partial V_{ij}(a,z)}{\partial d_{ij}/d_{ij}} -  \frac{\partial \Phi_{i}(a,z) / \Phi_{i}(a,z)}{\partial d_{ij}/d_{ij}}
\end{align}
where the key issue here is how a change in trade costs changes the choice specific value function. In other words, how much more valuable does choice $j$ become when it's cheaper to import variety $j$. And this is always compared to the change overall change in the value of options across variety, i.e., how $\Phi_{i}$ changes. In general, this is a complicated object as it is state dependent and is forward looking in with the value function showing up, not the period utility function. With that said, there are a couple of illustrative cases. 
\begin{itemize}
