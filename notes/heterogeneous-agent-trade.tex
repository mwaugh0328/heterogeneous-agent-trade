...
\end{align}
where $\mathbf{MPC}_{i}(a,z,j)$ is the household's marginal propensity to consume and $\gamma_{i}(a,z,j)$ is the Arrow-Pratt measure of relative risk aversion for a household in country $i$, states $a,z$, and consuming variety $j$. Now impose a specific preference structure, constant relative risk aversion (CRRA), and then $\gamma_{i}(a,z,j)$ becomes a constant $\gamma$ and log preferences is when $\gamma = 1$.
