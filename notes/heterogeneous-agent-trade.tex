...
\end{align}
Divide the stuff on the inside of the brackets by $p_{ij}c_{ij}(a,z)\pi_{ij}(a,z) \lambda(a,z)$ and multiply on the outside giving,
\begin{align}
...
\end{align}
Now define the following ``weight'' which is the share of goods that those with states $a,z$ account for in total expenditures from $j$ as
\begin{align}
...
\end{align}
where the elasticity of aggregate imports from $j$ is a weighted average of micro-level elasticities at the household level. And household level elasticities are decomposed into \textbf{(i)} an intensive margin trade elasticity which reflects how much more a household buys as trade costs decline; \textbf{(ii)} the extensive margin trade elasticity $\theta(a,z)_{ij}^{E}$ which reflects how households switch their variety choice as trade costs decline; and \textbf{(iii)} the distribution elasticity $\theta(a,z)_{ij}^{D}$ so how the mass of agents change across household states $a,z$.
...
\end{prp}
Proposition \ref{apx-prp:GET} is just derived off of the aggregation of imports at the micro level|no market clearing, functional forms, etc. However, by using the households budget constraint and the Type 1 extreme value distributional assumption on the choice probabilities more can be said about the intensive and extensive margin trade elasticity and illustrate how the household's consumption-savings problem matters.

\textbf{The Micro Intensive Margin Trade Elasticity.} By working off the budget constraint one can express the intensive margin elasticity as
\begin{align}
\underbrace{\frac{\partial p_{ij}c_{ij}(a,z)/ p_{ij}c_{ij}(a,z)}{\partial d_{ij} / d_{ij}}}_{\theta_{ij}(a,z)^{I}} &= -\frac{\partial a'(a,z)}{\partial p_{ij}/ p_{ij}} \frac{ \partial p_{ij}/ p_{ij}}{\partial d_{ij}/ d_{ij}} \times \frac{1}{p_{ij}c_{ij}(a,z)}, \\
\nonumber \\
&= -\frac{\partial a'(a,z)}{\partial p_{ij}/ p_{ij}} \times \frac{1}{p_{ij}c_{ij}(a,z)}.
\end{align}
where the last line follows from $\frac{ \partial p_{ij}/ p_{ij}}{\partial d_{ij}/ d_{ij}} = 1$. What this expression means is that the intensive margin elasticity is about a households consumption-savings behavior, i.e., how a household adjusts assets given a change in the prices of goods. For example, the way this works is that a reduction in trade costs lowers prices and relaxes the budget constraint, and then the intensive margin elasticity depends on how the incremental amount of resources available to spend is dived between assets and consumption. \textbf{need better notation as a' depends on ij}

\textbf{The Micro Extensive Margin Trade Elasticity.} The Type 1 extreme value distributional assumption allows for a characterization of the choice probability and, hence, how the choice probabilities respond to changes in trade costs. As a first step, define the denominator of the choice probability as:
\begin{align}
...
\end{align}
The key issue here is the first term: which reflects how the choice specific value function changes with trade costs. In other words, how much more valuable does choice $j$ become when it's cheaper to import variety $j$. The second term reflects the fact that the change choice $j$ depends upon a comparison relative to the overall change in the value of options across variety, i.e., how $\Phi_{i}$ changes.

In general, the first term in (\ref{eq:apx:ext-margin}) is a complicated object: is state dependent and forward looking in with the value function showing up. More subtle is the idea that the household's income fluctuations problem imposes a cross-equation restriction on between the intensive margin and extensive margin given how assets and consumption are responding to changes in prices via changes in trade costs.



\section{Appendix: The Gains from Trade}

This section works through the gains from a permanent change in trade costs, across steady states. So the idea here is, again, heuristic in the sense that I'm thinking a situation where the change is small and there is an immediate jump to the new steady state.

To walk through this I'm going to first express everything relative to the home country $i$. So the observation is that the expected value function over the taste shocks is
\begin{align}
...
\begin{align}
\mathbb{E}_{\epsilon} v_i(a, z) = -\sigma_{\epsilon} \log \pi_{ii}(a,z) + u(c_{ii}(a,z)) + \beta \mathbb{E} v_{i}(g_{ii}(a,z),z)
\label{aq:apx-home-valuefun}
\end{align}
where the expectation on the right-hand side is with respect to $z$ and $\epsilon$. The key innovation here is that now everything is written with respect to the home choice. Now the strategy is to differentiate (\ref{aq:apx-home-valuefun}) with respect to a percent change in trade costs and use the recursive structure to iterate forward and construct the overall change. 

Furthermore, in all of this, I set $w_i$ to be the numerarie and $A_i = 1$ so $p_{ii}$ equals one and needs not be carried around. 
...
\end{align}
which says that the agent should be indifferent between the marginal utility of consumption forgone to hold some more assets and two components (i) the benefit from changes in their choice probabilities  and (ii) the direct benefit of the returns on the assets evaluated at the marginal utility of consumption. \textbf{How is this consistent with Euler Equation below?}

Let's do this!
\begin{align}
\frac{\partial v_i(a, z)}{\partial d_{ij} / d_{ij}} = -\sigma_{\epsilon} \theta^{E}_{ij}(a,z) + u'(c_{ii}(a,z))\frac{\partial R}{\partial d_{ij} / d_{ij}}a - u'(c_{ii}(a,z))\frac{\partial g(a,z)}{\partial d_{ij} / d_{ij}}
+ \beta E \frac{\partial v_i(g(a,z), z')}{\partial d_{ij} / d_{ij}}
\end{align}
Then the derivative of the  continuation value function is:
\begin{align}
\frac{\partial v_i(g(a,z), z')}{\partial d_{ij} / d_{ij}} =& \underbrace{\bigg [-\sigma_{\epsilon} \frac{\partial \pi_{ii}(a',z') / \pi_{ii}(a',z')}{\partial a'} + u'(c_{ii}(a',z'))R \bigg ]}_{\frac{\partial v_i(g(a,z), z')}{\partial a}}\frac{\partial g(a,z)}{\partial d_{ij} / d_{ij}} + \\
\nonumber 
& -\sigma_{\epsilon} \theta^{E}_{ij}(a',z') + u'(c_{ii}(a',z'))\frac{\partial R}{\partial d_{ij} / d_{ij}}a' - u'(c_{ii}(a',z'))\frac{\partial g(a',z')}{\partial d_{ij} / d_{ij}}
+ \beta E \frac{\partial v_i(g(a',z'), z'')}{\partial d_{ij} / d_{ij}}
\end{align}
And now combine and collect terms so 
\begin{align}
\frac{\partial v_i(a, z)}{\partial d_{ij} / d_{ij}} =& -\sigma_{\epsilon} \theta^{E}_{ij}(a,z) \\
\nonumber \\
& + u'(c_{ii}(a,z))\frac{\partial R}{\partial d_{ij} / d_{ij}}a  \\
\nonumber \\
& + \bigg \{- u'(c_{ii}(a,z)) + \beta \mathbb{E} \big [-\sigma_{\epsilon} \frac{\partial \pi_{ii}(a',z') / \pi_{ii}(a',z')}{\partial a'} + u'(c_{ii}(a',z'))R \big ] \bigg \}\frac{\partial g(a,z)}{\partial d_{ij} / d_{ij}} \\
\nonumber \\
& + \beta \mathbb{E} \bigg \{ -\sigma_{\epsilon} \theta^{E}_{ij}(a',z') +  u'(c_{ii}(a',z'))\frac{\partial R}{\partial d_{ij} / d_{ij}}a' \ldots 
\end{align}
...
\item $u'(c_{ii}(a,z))\frac{\partial R}{\partial d_{ij} / d_{ij}}a$ is what I would call how trade facilitates more risk sharing. Would need to compute this...I think as prices decline the amount of resources available for risk sharing expands and this changes the interest rate leading to a gain on asset holdings, may lead to a loss for those owing money.
    
\item The third term is interesting because it is the stuff in the outer brackets is the Euler equation for assets. The idea here is that if the household is unconstrained, then it is zero as there is no gain through changes in asset behavior. Asset holdings are already chosen so that margins are equated, thus on the margin how this changes asset behavior is zero. 
...
\item The final term is about this continuing on so the final form is the expected discounted some thing...
\end{itemize}
