...
What seems relatively non-standard is that the marginal utility of consumption times consumption shows up. One way to view this part of the elasticity is that it's the rate of conversion from quantities into utils and if one inserts it into equation (\ref{eq:eff-trade-gains}) this becomes clear. In the efficient allocation, this rate of conversion reflects consumption times the number of utils deliver at that level which is the marginal utility of consumption.
...
\end{itemize}
\end{prp}
Before moving on, it's worth unpacking the gains from substitution term in the following way. The change in the home share (at any date, so I'm going to drop the time subscript) can be expressed as
\begin{align}
-\sigma_{\epsilon} \frac{\mathrm{d} \pi_{ii}(a,z) / \pi_{ii}(a,z) }{\mathrm{d} d_{ij} / d_{ij}} = \sum_{j'} \pi_{ij'}(a,z) \bigg[ \frac{\mathrm{d} v_{i}(a,z,j')}{\mathrm{d} d_{ij}/d_{ij}} - \frac{\mathrm{d} v_{i}(a,z,i)}{\mathrm{d} d_{ij}/d_{ij}} \bigg]. \label{eq:home-choice-change}
\end{align}
which is a weighted average of how value functions change relative to the home and the weights reflect the choice probabilities. Here two things matter. The obvious one is through exposure and that is all baked into the choice probabilities. If different household have different choice probabilities, then this force leads to heterogenous gains from trade through the substitution term.\footnote{To make this concrete, thinking through the patterns in Figure \ref{fig:micro-trade}, poor households' have near zero exposure and, hence, the change in \ref{eq:home-choice-change} is zero | even if value functions are moving a lot.}

The bracketed term in (\ref{eq:home-choice-change}) is where elasticities enter. To see this, define $\bar{\theta}(a,z) ^E_{ij,j'}$ as the extensive margin, cross-price elasticity, and in total derivative form. The expression for $\bar{\theta}(a,z) ^E_{ij,j'}$ takes the same form as the own-price expressions in (\ref{eq:extensive-margin}). Inserting this definition into (\ref{eq:home-choice-change}) gives
\begin{align}
-\sigma_{\epsilon} \frac{\mathrm{d} \pi_{ii}(a,z) / \pi_{ii}(a,z) }{\mathrm{d} d_{ij} / d_{ij}} = \sigma_{\epsilon} \sum_{j'} \pi_{ij'}(a,z) \bigg[ \bar{\theta}(a,z) ^E_{ij,j'} - \bar{\theta}(a,z) ^E_{ii,j'} \bigg].
\end{align}
and now the bracketed term reflects relative cross-price elasticities. 
...
\end{align}
Then using my H-A Trade Elasticity formula and canceling terms (noticing as well that the expenditure weights don't matter since they are common across households) I have that:
\begin{align}
