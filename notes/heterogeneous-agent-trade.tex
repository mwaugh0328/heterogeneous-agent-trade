...
\item Do steps \textbf{2.} and \textbf{3.} for each $j$ variety choice. This then makes the function $g_{a,ij}(a,z)$ mapping each state and $j$ choice (today) into $a', z'$ states.

\item[\textbf{3.}] Compute the transition probability matrix ($Q$ in my notation) that characterizes transitions from $a,z$ into $a',z'$. \textbf{Note $Q$ does not depend on $j$.} So this can be done in one whole step.
