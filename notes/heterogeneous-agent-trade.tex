...
I'm going to walk through a couple of measures of welfare.

\subsection{Equivalent Variation}

Equivalent variation does the following. Let's suppose some price change gets us to utility level $u_1$, the equivalent variation asks ``at the old prices, $p_0$, how much extra income must I provide you to be indifferent between $u_1$ and $u_0$.''

Now, let's do this in my model. Define the value function of a household in the base period as
\begin{align}
v_i(a, z ; \ p_{ij}, R_{i}, w_{i})
\end{align}
And then define the value function for the same states, but at some counter factual prices
\begin{align}
v'_i(a, z ; \ p'_{ij}, R'_{i}, w'_{i})
\end{align}
Then my equivalent variation measure is a transfer $\tau$, in the base period at the old prices such that the new level of utility $v'_i$ is achieved:
\begin{align}
EV = v'_i - v_i(a, z ; \ p_{ij}, R_{i}, w_{i}, \ \tau(a,z)) = 0.
\end{align}
where I'm treating the $v'_i$ part like a number and the question is how do I get there. \textbf{Choices.} This all sounds straight forward, but how do you give the $\tau$ both in the units and temporally.  Here is what I'm currently dong:
\begin{itemize}
\item My Lucas-style equivalent variation finds the $\tau(a,z)$ at the old prices to meet the new level of utility. The $\tau$ is a proportional increase in consumption of all commodities. This is relatively simple because I can fix policy functions at the old prices and reconstruct the value function simply scaling consumption in every state, every commodity by $\tau(a,z)$ and then find the proportional increase
\end{itemize}
