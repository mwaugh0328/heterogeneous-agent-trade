...
{\footnotesize
\begin{align}
\theta_{ij} = \sum_{a,z} \bigg \{ \theta_{ij}(a,z)^{I} + \theta_{ij}(a,z)^{E} \bigg \}\omega_{ij}(a,z) - \bigg \{ \theta_{ii}(a,z)^{I} + \theta_{ii}(a,z)^{E} \bigg \}\omega_{ii}(a,z)
...
The feature that's shaping micro responses at the intensive margin is market incompleteness and a households ability to partially insure. In the efficient allocation discussed above, the intensive margin elasticity is zero as the planner correctly reallocates resources across households and any change in trade costs is completely offset by expenditures. In the decentralized allocation, the household is on its own and must judge how to reallocate gains from a reduction in prices via trade costs between current consumption versus the future. Heterogeneity matters because the trade-off current consumption versus the future is not invariant to a household's state $a, z$.
...
\begin{figure}[!t]
\centering{\includegraphics[scale = 0.58]{./figures/micro-elasticity.pdf}}
\caption{Micro-level Trade Elasticities, $\theta_{ij}(a,z)$}\label{fig:auto_sales_diff_diff}
\end{figure}


\begin{figure}[!t]
\centering{\includegraphics[scale = 0.58]{./figures/trade-share.pdf}}
\caption{Micro-level Trade, $M_{ij}(a,z) / M_{ii}(a,z)$ }\label{fig:auto_sales_diff_diff}
\end{figure}
...
\begin{align}
W = E\sum_{t=0}^{\infty}  \sum_{i} \sum_{j} \sum\limits_{z}  \beta^{t} \  \bigg \{  u(c_{ij}(z, t) ) + \mathrm{E}[ \ \epsilon \ | \ \pi_{ij}(z,t) ] \bigg \}\pi_{ij}(z,t) L_{i} \lambda_{i}(z, t)
...
\textbf{The Elasticity of Trade.} This is a bit non-standard from my perspective, but here is my argument. Claim \#1: The intensive margin trade elastic is zero, i.e. any change in $d_{ij}$ results in $p_{ij}c_{ij}$ to zero, where the $p$'s would be reflecting the correct multipliers. So I don't need to mess with this.
