...
\item[\textbf{1.}] Given the consumption function and choice probabilities compute the RHS of (\ref{eq:euler_equation}) and then invert to find the new updated consumption choice so
\begin{align}
...
\end{align} 
where $u^{' -1}$ is the inverse function of the marginal utility of consumption. Side note: one of the interesting things about this equation is that the direct $j$ component on the RHS that only affects the consumption choice is through the price. Can this be exploited? Also reveals the intensive margin price elasticity.

\item[\textbf{2.}] The key issue in this method is that we have found  $c_{ij}(\tilde a, z)$ where the consumption function is associated with some asset level that is not necessarily on the grid. The solution is to (i) use the budget constraint and infer $\tilde a$ given that $a'$ was chosen above, $z$, and $c_{ij}(\tilde a, z)$. Now we have a map from $\tilde a$ to $a'$ for which one can use interpolation to infer the $a'$ chosen given $a$ where $a$ is on the grid.
    
\item[\textbf{3.}] Given the map from $a$ to $a'$ (conditional on a product choice and $z$) infer the transition probability function ($Q_{ij}$ in my notation) that characterizes transitions from $a,z$ into $a',z'$ and here this would have an additional twist in that it would also reflect the choice probabilities $\pi_{ij}(a', z')$. It is also dependent on the $j$ product choice, so there is one of these for each $j$.
    
\item[\textbf{4.}] Given $Q_{ij}$, one can compute the value functions $v_{ij}$ and then new choice probabilities from \ref{eq:choice-prob}. 

\item[\textbf{5.}] Compare old and new policy functions, old and new choice probabilities, and then update accordingly. 
