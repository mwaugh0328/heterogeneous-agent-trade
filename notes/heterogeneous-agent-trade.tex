...
The final column reports the primitive estimates on the trade cost function. Each value reports the level effect of being in a distance bin or sharing a border etc.  So if a two countries are measured to be in the smallest distance bin and share a border, the trade cost between these two countries is $2.35 \times 0.91$ (first row times seventh row). Or if a country is in the furthest distance bin, its trade costs is 5.69.
...
This final section performs computes what the patter of trade \emph{should} look like were we living in a first best world.
...
An preliminary step to computing the allocation in Proposition \ref{prp:efficient-allocation} is to take a stand on Pareto weights.

My desire here is to choose weights so the planner is not drastically shifting consumption across countries to equate margins and the key margin is equating margins within the country. To that end, I set the Pareto weight on the population in Country $i$ to be proportional to it's level of TFP. This specification limits the planners desire to redistribute consumption from say, the US (a rich country) to Greece (a poor country in the data set).

\subsection{Efficient Trade}

Figure \ref{fig:planner-trade} provides a first look as to what the efficient pattern of trade would look like.

On the x-axis of Figure \ref{fig:planner-trade} is log bilateral trade flows in the model (and data) in the decentralized equilibrium. So the same thing as in Figure \ref{fig:model-fit}. The y-axis reports the trade flows for each bilateral pair in the efficient allocation. Then the 45-degree line goes a long diagonal. If the red-dots lie on this line, this would mean that trade in the efficient allocation lines up with the decentralized allocation.

Generally they do not line up on the 45-degree line. Two aspects of this stand out. First, smaller trade flows tends to be, to small. From the planners perspective these are not efficient trading relationships. In contrast, relatively large trading relationships are generally too small, this is the set of dots lying from about -5 to 0. So is a real sense that the planer would want to reallocate how households source goods.
...
Figure \ref{fig:us-data-trade} takes a deep dive to illustrate this reallocation by focusing on how US imports change. The left panel reports import shares in the model (and data). Here you can see that the aggregate import share is about 7 percent with Canada, Japan, and some other European countries contributing to the US's sourcing pattern.
...
The issue is this: far away, unproductive source countries are ``luxuries'' from the planners perspective. In the decentralized equilibrium, the varieties from these countries are only purchased from rich households (and have the appearance of being luxuries as they are also high elasticity goods). Then, when the planner equalizes consumption within a country, the demand for these goods is eliminated.

Why does trade expand in certain directions? This is not obvious to me either, but I think there is an optimistic message here: the US is under-utilizing other productive countries via trade as a means to increase consumption for the masses. This is why trade expands with Japan which in the calibrated economy is very productive with a large labor force.
