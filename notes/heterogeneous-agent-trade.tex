...
This example is interesting because it retains a constant trade elasticity and then the welfare gains formula looks like ACR+ kind of thing. There is a bit more to it, but a key insight is that policy function does not depend upon the variety choice and this implies that choice probabilities are independent of $z$, there is a constant trade elasticity as the intensive margin is turned off, and then the gains from trade take a bit cleaner formulation in Proposition \ref{apx-prp:gains-trade}.
...
\end{align}
which then leads to the observation that the optimal $a'$ conditional on a choice $j$ is \textbf{independent} of the price of the consumption good. So what is going on is if you consume an expensive or cheap good, then consumption simply scales up or down so that assets next period are exactly the same. 

This observation has the other following implication. Compare households expenditures with the same state $a,z$ but different choices. Equation (\ref{eq:value_fun_option_log_p}) implies
\begin{align}
...
Finally notice that the choice probability becomes
\begin{align}
...
\end{align}
which is exactly the same as discussed above in the static model.

\textbf{Step 2: Micro Trade Elasticities.} I'm going to be super systematic about this:
\begin{itemize}
\item Starting with (\ref{eq:apx-intensive-margin}) and because the asset choice is independent of prices,  the intensive margin elasticity $\theta_{ij}(a,z)^I$ is zero. 
...
\end{align}
where the first line removes the $a,z$ indexing of $\Phi_i$ because they don't shape the choice probabilities. The next line then partially differentiates the value function with respect to the change in trade costs where I'm exploiting how with log preferences one can pull out the price term. And then the final line notes that the price elasticity is one. Now here is one more key fact that:
\begin{align}
\theta_{ii}(a,z)^E =&  \beta \mathbb{E} \frac{\partial v_{i}(a',z')}{\partial d_{ij}/d_{ij}} -  \frac{\partial \Phi_{i} / \Phi_{i}}{\partial d_{ij}/d_{ij}}
\end{align}
where a key thing to notice is that the $i,i$ elasticity is the same as the second terms above in (\ref{eq:apx-log-partial-valuefun}). This is important to keep in mind.
\end{itemize}

\textbf{Step 3: The Expenditure Weights.} Recall that 
\begin{align}
\omega_{ij}(a,z) = \frac{p_{ij}c_{ij}(a,z)\pi_{ij}(a,z) \lambda_{i}(a,z)}{M_{ij}}.
\end{align}
and note that we can relabel $p_{ij}c_{ij}(a,z) = x(a,z)$ given the fact discussed above in (\ref{eq:apx-same-spending}). Then with the choice probabilities being independent of $a,z$ this becomes
\begin{align}
...
\begin{align}
\nonumber
\theta_{ij} =& \sum_{a,z} \bigg \{ 0 + \frac{1}{\sigma_{\epsilon}} + \beta \mathbb{E} \frac{\partial v_{i}(a',z')}{\partial d_{ij}/d_{ij}} -  \frac{\partial \Phi_{i} / \Phi_{i}}{\partial d_{ij}/d_{ij}} \ + 0 \bigg \}\omega_{i}(a,z) - \sum_{z} \bigg \{  0 + \beta \mathbb{E} \frac{\partial v_{i}(a',z')}{\partial d_{ij}/d_{ij}} -  \frac{\partial \Phi_{i} / \Phi_{i}}{\partial d_{ij}/d_{ij}} + 0 \bigg \}\omega_{i}(a,z) \\
\nonumber \\
