...
\end{align}
Then divide the stuff on the inside of the brackets by household level imports $p_{ij}c_{ij}(a,z)\pi_{ij}(a,z) \lambda(a,z)$ and multiply on the outside giving,
\begin{align}
...
\end{prp}
Proposition \ref{apx-prp:GET} is just derived off of the aggregation of imports at the micro level|no market clearing, functional forms, etc. However, by using the households budget constraint and the Type 1 extreme value distributional assumption more can be said about the intensive and extensive margin trade elasticity and illustrate how the household's consumption-savings problem matters.
...
\end{align}
where recall that $g_{ij}(a,z)$ is the policy function mapping states into asset holdings next period $a'$  and the last line follows from $\frac{ \partial p_{ij}/ p_{ij}}{\partial d_{ij}/ d_{ij}} = 1$. What this expression means is that the intensive margin elasticity is about a households consumption-savings behavior, i.e., how a household adjusts assets given a change in the prices of goods. For example, the way this works is that a reduction in trade costs lowers prices and relaxes the budget constraint, and then the intensive margin elasticity depends on how the incremental amount of resources available to spend is dived between assets and consumption. 

A couple of notes:
\begin{itemize}
\item Is there a way to connect this with the marginal propensity to consume (or save)?
\item I'm doing everything as if it's discrete. Will need to change notation above. 
\end{itemize}
...
In general, the first term in (\ref{eq:apx:ext-margin}) is a complicated object: is state dependent and forward looking in with the value function showing up. More subtle it is the idea that the household's income fluctuations problem imposes a cross-equation restriction on between the intensive margin and extensive margin given how assets and consumption are responding to changes in prices via changes in trade costs.
...
\end{align}
where the expectation on the right-hand side is with respect to $z$ and $\epsilon$. The key innovation here is that now everything is written with respect to the home choice. Now the strategy is to differentiate (\ref{eq:apx-home-valuefun}) with respect to a percent change in trade costs and use the recursive structure to iterate forward and construct the overall change.
...
\end{align}
which says that the agent should be indifferent between the marginal utility of consumption forgone to hold some more assets and two components (i) the benefit from how a change in assets changes in their variety choice and (ii) the direct benefit of the returns on the assets evaluated at the marginal utility of consumption. Something that is confusing me is how is this consistent with Euler Equation below for the endogenous grid method. 

Let's do this!
\begin{align}
\frac{\partial v_i(a, z)}{\partial d_{ij} / d_{ij}} = -\sigma_{\epsilon} \theta^{E}_{ii}(a,z) + u'(c_{ii}(a,z))\frac{\partial R}{\partial d_{ij} / d_{ij}}a - u'(c_{ii}(a,z))\frac{\partial g_{ii}(a,z)}{\partial d_{ij} / d_{ij}}
+ \beta E \frac{\partial v_i(g_{ii}(a,z), z')}{\partial d_{ij} / d_{ij}}
\end{align}
Side note: I think I'm conflating total derivatives and partial derivatives... so the notation is a bit messed up. Then the derivative of the continuation value function is:
\begin{align}
\frac{\partial v_i(g(a,z), z')}{\partial d_{ij} / d_{ij}} =& \underbrace{\bigg [-\sigma_{\epsilon} \frac{\partial \pi_{ii}(a',z') / \pi_{ii}(a',z')}{\partial a'} + u'(c_{ii}(a',z'))R \bigg ]}_{\frac{\partial v_i(g_{ii}(a,z), z')}{\partial a}}\frac{\partial g_{ii}(a,z)}{\partial d_{ij} / d_{ij}} + \\
\nonumber
& -\sigma_{\epsilon} \theta^{E}_{ij}(a',z') + u'(c_{ii}(a',z'))\frac{\partial R}{\partial d_{ij} / d_{ij}}a' - u'(c_{ii}(a',z'))\frac{\partial g_{ii}(a',z')}{\partial d_{ij} / d_{ij}}
+ \beta E \frac{\partial v_i(g_{ii}(a',z'), z'')}{\partial d_{ij} / d_{ij}}
\end{align}
...
\nonumber \\
& + \underbrace{u'(c_{ii}(a,z))\frac{\partial R}{\partial d_{ij} / d_{ij}}a}_{\gamma_{ii}(a,z)}  \\
\nonumber \\
& + \underbrace{\bigg \{- u'(c_{ii}(a,z)) + \beta \mathbb{E} \big [-\sigma_{\epsilon} \frac{\partial \pi_{ii}(a',z') / \pi_{ii}(a',z')}{\partial a'} + u'(c_{ii}(a',z'))R \big ] \bigg \}\frac{\partial g_{ii}(a,z)}{\partial d_{ij} / d_{ij}}}_{\delta_{ii}(a,z)} \\
\nonumber \\
...
\subsection{Static, Log-Logit, \citet{arkolakis2012new}}
