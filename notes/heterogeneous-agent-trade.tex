...
\begin{enumerate}
\item \textbf{What are the model's implications for trade elasticities and the gains from trade in decentralized allocation?} Here I characterize micro and macro level trade elasticities and then the gains from trade and I connect them with heterogeneity in the marginal utility of consumption that is induced by market incompleteness. I explore two special cases: (i) the efficient allocation and (ii0 when utility is $\log$ over consumption and how this case delivers separation between the heterogenous agent side of the economy and trade.
...
The first column of Table \ref{tb-micro-shares} reports the resulting elasticities. These are right in the ballpark, but perhaps understate the heterogeneity in elasticities as rich households are more elastic in my model than found in \citet{auer2022unequal}.

The second column of Table \ref{tb-micro-shares} reports how household-level import shares vary across the income distribution. The share of imports out of total expenditure for households below median income is the same as those above the median. There is some non-monotonicity which is associated with the imperfect correlation between income and assets holdings and that quality shifters only work though income. With that said, the overall pattern is consistent with the facts of as found in \citet{borusyak2021distributional} and if there is any bias it is for poor households to experience less gains from trade as they are slightly less exposed relative to the median household.
...
\\\hline \hline
& Trade Elasticities & Import Shares  & MPCs\\
\cmidrule(lr){2-2} \cmidrule(lr){3-3} \cmidrule(lr){4-4}
Below Median Income       & $-6.56$  & $0.06$  & 0.50\\
Median                    & $-4.60$  & $0.08$  & 0.28\\
Above Median Income       & $-3.40$  & $0.06$  & 0.17\\
\hline
\end{tabular}
\\[0.5ex]
%\parbox{5.95in}{\footnotesize \textbf{Note:} }
