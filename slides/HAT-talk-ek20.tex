...
\begin{align*}
\mathrm{E}\sum_{t = 0}^{\infty} \beta^{t} \ \sum_{j} \mathbbm{1}^{k}_{jt} \ \tilde{u}^k_{ijt}
\end{align*}
where $\mathbbm{1}^{k}_{jt}$ toggles on/off depending on good chosen and conditional direct utility for good $j$ is
\begin{align*}
...
\bigskip
\medskip
Just like \citet{eaton2002technology}! It's all about how this price-index-like thing changes.
}
\only<2>{
\begin{align*}
\frac{\mathrm{d} v_i(a, z)}{\mathrm{d} d_{ij} / d_{ij}} \approx \sum_{j} \pi_{ij}(a, z) \frac{\mathrm{d} v_{ij}(a,z)}{\mathrm{d} d_{ij}/d_{ij}}
\end{align*}\\
\bigskip
\medskip
...
\only<3>{
\begin{align*}
\frac{\mathrm{d} v_i(a, z)}{\mathrm{d} d_{ij} / d_{ij}} \approx \underbrace{\sum_{j} \pi_{ij}(a, z) \bigg \{ \frac{\mathrm{d} v_{ij}(a,z)}{\mathrm{d} d_{ij}/d_{ij}} - \frac{\mathrm{d} v_{ii}(a,z)}{\mathrm{d} d_{ij}/d_{ij}} \bigg \}}_{\ \ \mbox{\small how relative valuations change\phantom{foo}}} \ + \  \frac{\mathrm{d}v_{ii}(a,z)}{\mathrm{d} d_{ij}/d_{ij}}
\end{align*}
}
\only<4>{
\begin{align*}
\frac{\mathrm{d} v_i(a, z)}{\mathrm{d} d_{ij} / d_{ij}} \approx \underbrace{\sum_{j} \pi_{ij}(a, z) \bigg \{ \frac{\mathrm{d} v_{ij}(a,z)}{\mathrm{d} d_{ij}/d_{ij}} - \frac{\mathrm{d}v_{ii}(a,z)}{\mathrm{d} d_{ij}/d_{ij}} \bigg \}}_{ = \ \mbox{\small how the home choice, $\pi_{ii}$, changes} } \ + \  \frac{\mathrm{d}v_{ii}(a,z)}{\mathrm{d} d_{ij}/d_{ij}}
\end{align*}\\
\bigskip
\medskip
...
\only<5>{
\begin{align*}
\frac{\mathrm{d} v_i(a, z)}{\mathrm{d} d_{ij} / d_{ij}} \approx \mathbb{E}_{z} \sum_{t = 0}^{\infty} \beta^{t} \bigg \{ -\sigma_{\epsilon} \frac{\mathrm{d} \pi_{ii}(a_{t},z_{t}) / \pi_{ii}(a_{t},z_{t})}{\mathrm{d}d_{ij} / d_{ij}} + u'(c_{ii}(a_{t},z_{t}))\bigg[ \ a_{t} \ \frac{\mathrm{d} R_{i} / p_{ii}}{\mathrm{d} d_{ij} / d_{ij}} \ \bigg] \bigg \}.
\end{align*}\\
\bigskip
\medskip
HH-level gains pick up two effects:
\begin{itemize}
\item An ACR-like term summarizing how relative valuations across choices change.
\smallskip
\item How hh's real wealth ($+$ or $-$) change through GE effects on prices | all evaluated at the hh's marginal utility of consumption.
\end{itemize}
}
\end{frame}
...
\begin{align*}
\frac{\mathrm{d} v_i(a, z)}{\mathrm{d} d_{ij} / d_{ij}} \approx \underbrace{-\frac{1}{\theta (1-\beta)} \times \frac{\mathrm{d} \pi_{ii} / \pi_{ii}}{\mathrm{d}d_{ij} / d_{ij}}}_{\mbox{\small ACR}} \ \ + \  \ \underbrace{\mathbb{E}_{z} \sum_{t = 0}^{\infty} \beta^{t} \bigg[ u'(c_{ii}(a_{t},z_{t})) \ a_{t} \ \frac{\mathrm{d} R_{i} / p_{ii}}{\mathrm{d} d_{ij} / d_{ij}} \bigg ]}_{\mbox{\small with the magic of $\log$, turns out this} \ = \ 0}.
\end{align*}\\}
\medskip
...
\only<2>{And we are back to \citet{arkolakis2012new}\ldots}
\end{frame}
...
\smallskip
\item Do all of this in Financial Globalization case...no balance of trade.
\end{itemize}
\bigskip
...
\begin{figure}[!t]
\centering{\includegraphics[scale = .35]{../notes/figures/trade-fit-fg.pdf}}
\end{figure}
\end{frame}
...
\end{itemize}
\bigskip
\textbf{2.} \textbf{GE effects create winners and losers}
\begin{itemize}
\item Who benefits from \textbf{1.} + effects on $R/p$ $\Rightarrow$ shapes the extent to which their are winners and losers.
\end{itemize}
\bigskip
...
%\begin{figure}[t]
%\centering{\includegraphics[scale = .45]{../notes/figures/japan-trade.pdf}}
%\end{figure}
%\end{frame}

\begin{frame}[t]{U.S. Welfare: 10\% Reduction to Japan, {\color{red} Fixed $R$ \& $w$} }
\begin{figure}[!t]
\centering{\includegraphics[scale = .43]{../notes/figures/welfare-jpn-fix-p.pdf}}
\end{figure}
\end{frame}

\begin{frame}[t]{U.S. Welfare: 10\% Reduction to Japan, {\color{red} GE} }
\begin{figure}[!t]
\centering{\includegraphics[scale = .43]{../notes/figures/welfare-jpn-ge.pdf}}
\end{figure}
\end{frame}
...
\footnotesize  Bottom quartile  & 0.06 & -0.028 \\
\footnotesize  Median & 0.08 & 0.02 \\
\footnotesize  Upper quartile  & 0.011 & 0.10  \\
\footnotesize  Aggregate & 0.09 & 0.02\\
\cmidrule(lr){2-2}  \cmidrule(lr){3-3}
\footnotesize  \% losers & 0.0 & 37.1\\
\hline
\end{tabular}
...
%
%\begin{figure}[t]
%\centering{\includegraphics[scale = .45]{../notes/figures/canada_trade.pdf}}
%\end{figure}
%\end{frame}

\begin{frame}[t]{U.S. Welfare: 10\% Reduction to Canada, {\color{red} Fixed $R$ \& $w$} }
\begin{figure}[t]
\centering{\includegraphics[scale = .43]{../notes/figures/welfare-can-fix-p.pdf}}
\end{figure}
\end{frame}

\begin{frame}[t]{U.S. Welfare: 10\% Reduction to Canada, {\color{red} GE } }
\begin{figure}[!t]
\centering{\includegraphics[scale = .43]{../notes/figures/welfare-can-ge.pdf}}
\end{figure}
\end{frame}
...
\cmidrule(lr){2-2}  \cmidrule(lr){3-3}
\footnotesize  Asset Quartile & \footnotesize  Welfare (\% Change) & \footnotesize  Welfare (\% Change) \\
\footnotesize  Bottom quartile  & 0.21 & 0.07 \\
\footnotesize  Median & 0.28 & 0.09 \\
\footnotesize  Upper quartile  & 0.39 & 0.13  \\
\footnotesize  Aggregate & 0.30 &  0.10 \\
\cmidrule(lr){2-2}  \cmidrule(lr){3-3}
...
\textbf{2.} \textbf{GE effects create winners and losers}\\
\bigskip
1 + 2 leads to the idea that the being exposed / liberalizing with some markets is better than others.\\
\bigskip
\medskip
This shows up in the planners allocation. Next two slides..
\begin{itemize}
\item Trade in the decentralized equilibrium vs.
\smallskip
\item the Planner's allocation
\end{itemize}
\end{frame}
...
\begin{figure}[t]
\centering{\includegraphics[scale = .55]{../notes/figures/decentralized-trade-us.pdf}}
\end{figure}
\end{frame}
...
\begin{figure}[t]
\centering{\includegraphics[scale = .55]{../notes/figures/planner-trade-us.pdf}}
\end{figure}
\end{frame}
...
\bigskip
\textbf{1.} Use gravity model + indirect inference to estimate the model. And better confront micro-evidence.
\begin{itemize}
\smallskip
\item ``Gravity as a guide, not as a law''
\end{itemize}
\bigskip
\textbf{2.} Can trade policy improve outcomes?
\begin{itemize}
\smallskip
\item Put tariffs in and redistribute
\end{itemize}
\end{frame}
...
\bigskip
\bigskip
\huge{Thank You Jonathan and Sam!!!!!!}
...
\begin{frame}[t]{Financial Autarky, Bilateral Trade: Model vs. Data}
\begin{figure}[!t]
