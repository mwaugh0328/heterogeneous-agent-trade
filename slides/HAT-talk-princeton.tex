...
\end{itemize}
\bigskip
A generalization with multiple sectors is the ``infinite shopping aisle'' approach we use in\\ \citet{p-iq}.
\end{frame}
...
\smallskip
\only<1>{
The problem: no closed form map from trade flows to parameters as in static, trade models do. But I want the model to replicate the geographic pattern of activity seen in the data.}
...
\end{align}
\item Step 1. Run this gravity regression (same as in \citet{eaton2002technology}) on the data 
\begin{align}
