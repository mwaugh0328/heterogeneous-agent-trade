...
%\begin{frame}[t]{The Contribution and Agenda\ldots}
%\smallskip
%Thomas Paine in opening line of ``Common Sense'':\\
%``\emph{a long habit of not thinking a thing \textbf{wrong}, gives it a superficial appearance of being \textbf{right}}''\\
%\bigskip
%CES is nice, but abstracts from empirically relevant issues about how household heterogeneity shapes demand.
%\begin{itemize}
%\item This paper is about trade and the gains from trade.
%\smallskip
%\item Pricing and market power issues in \citet{mongey-waugh-1}.
%\smallskip
%\
%\end{itemize}
%\end{frame}
...
\medskip
$k = {1,\ldots, K}$ types of agents and mass $\mu^k$ of each type. Agents are heterogenous in wealth $w^k$.\\
\smallskip
Agents have preferences:
\begin{align*}
u(c_{hh}) + \epsilon_{h}  \ \ \ \  \mbox{and} \ \ \ \ u(c_{hf}) + \epsilon_{f},
\end{align*}
\begin{itemize}
\item discrete choice\ldots so each agent chooses \textbf{\emph{only}} the home or foreign variety.
\smallskip
\item $\epsilon_{j}$s are iid across agents and options, distributed Type 1 EV with parameter $\sigma_{\epsilon}$.
\smallskip
...
\bigskip
\medskip
Aggregate trade aries from the \begin{alert}{\textbf{explicit aggregation}}\end{alert} of these decisions
\begin{align*}
M_{hf} = \sum\limits_{k\in K} w^k \pi_{hf}^k \mu^k.
\end{align*}\\
\bigskip
The \begin{alert}{\textbf{aggregate trade elasticity}}\end{alert} is just a expenditure weighted average of all agent $k$s elasticities.
\begin{itemize}
...
\smallskip
\item Intuition: A price reduction delivers a lot of extra utility for high marginal utility ( poor ) agents and this induces strong substitution by the poor.
\end{itemize}
\bigskip
...
\smallskip
\item \begin{alert}{\textbf{Here elasticities matter}}\end{alert}. And under certain conditions, these gains are pro-poor.
\end{itemize}
\medskip
...
\begin{itemize}
\item \textbf{Theory:} Very salient in spatial models ( {\small \citet*{miyauchi2024unpacking}} ) and connects with the complete markets decentralization of discrete choice models in {\small \citet{mongey-waugh-2}}.
\smallskip
...
\bigskip
\textbf{2.} The standard incomplete markets model on the household side. This is quantitatively important because\ldots
\begin{itemize}
\smallskip
\item In the illustrative model, there was no ability to insure,
\smallskip
\item and types are fixed.
\end{itemize}
\medskip
The SIM model has self-insurance and household-level dynamics (poor people today are rich in the future). Both these forces work \begin{alert}{\textbf{against}}\end{alert} the mechanisms I just described.
\end{frame}
...
\only<4>{
\bigskip
This term is about how assets adjust
\vspace{.15cm}
{\small
\begin{align*}
C(a,z) = \bigg \{\underbrace{ - \frac{u'(c_{i}(a,z,i))}{p_{ii}} + \beta \mathbb{E}_{z'} \bigg [-\sigma_{\epsilon} \frac{\partial \pi_{ii}(a',z') / \pi_{ii}(a',z')}{\partial a'} + \frac{u'(c_{i}(a',z',i))R_{i}}{p_{ii}} \bigg ] }_{\mbox{Euler equation}} \  \bigg \}\frac{\mathrm{d} a'_{i}(a,z,i)}{\mathrm{d} d_{ij} / d_{ij}}
\end{align*}
}
\\
\medskip
which is \begin{alert}{\textbf{zero}}\end{alert} for small changes as hh's are either (i) on their Euler equation or (ii) constrained and can't adjust their asset position.
}
\end{frame}
...
\only<2>{And we are back to \citet{arkolakis2012new} $+$ what's going on with valuation effects and asset adjustment.}
\end{frame}
...
Slope of Quality Shifter, \ $\psi_{ii}(z)$          & --- & \phantom{\} } Micro moments of {\small \citet{borusyak2021distributional} } \\
...
\bigskip
\bigskip
Today, I'm doing this across steady states, not transition path.
\begin{itemize}
\item I just computed the path last week, need time to think about results\ldots sorry :(
\end{itemize}
\end{frame}
